...
class Data:
    def __init__(self):
        ...
        self.pretrain_label_embedding = None
        ...
        self.mem_bank_dropout = 0.2
        self.mem_bank_alpha = 0.3
        self.global_hidden_dim = 128  
        self.global_kernel_size = 55
        ...
        self.use_crf = True
        self.nbest = None
        self.global_feature_extractor = "LSTM"
        ...
        ### Hyperparameters
        self.HP_cnn_layer = 4
        self.HP_intNet_layer = 5
        self.HP_intNet_kernel_type = 2
    ...
    def show_data_summary(self):
        ...
        if self.label_emb_dir is not None:
            print("   Label embedding  dir: %s"%(self.label_emb_dir))
        ...
        print("         Memory mat number: %s"%(len(self.mem_mat)))
        print("        Memory word number: %s"%(len(self.word_mat)))
        ...
        if self.use_char:
            ...
            print("     Model char cnn window: %s"%(self.char_cnn_win))
            print("   Model char intNet layer: %s" %(self.HP_intNet_layer))
            print("   Model char intNet kernel type: %s" %(self.HP_intNet_kernel_type))
            print("     Global word extractor: %s"%(self.global_feature_extractor))
            print("         Global hidden_dim: %s"%(self.global_hidden_dim))
        print(" "+"++"*20)
        print(" Training:")
        ...
        print("     Hyper    rnn  dropout: %s"%(self.rnn_dropout))
    ...
    def build_pretrain_emb(self):
        ...
        if self.label_emb_dir:
            print("Load pretrained label embedding, norm: %s, dir: %s"%(self.norm_word_emb, self.label_emb_dir))
            self.pretrain_label_embedding, self.label_emb_dim = build_pretrain_embedding(self.label_emb_dir, self.label_alphabet, self.word_emb_dim, self.norm_word_emb)
    ...
    def generate_instance(self, name):
        ...
        if name == "train":
            self.train_texts, self.train_Ids, self.mem_mat, self.word_mat, self.word_idx = read_instance(self.train_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.sentence_classification, self.split_token, self.word_idx, self.word_mat, self.mem_mat)
        elif name == "dev":
            self.dev_texts, self.dev_Ids, self.mem_mat, self.word_mat, self.word_idx = read_instance(self.dev_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, -1, self.sentence_classification, self.split_token, self.word_idx, self.word_mat, self.mem_mat )
        elif name == "test":
            self.test_texts, self.test_Ids, self.mem_mat, self.word_mat, self.word_idx = read_instance(self.test_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, -1, self.sentence_classification, self.split_token, self.word_idx, self.word_mat, self.mem_mat )
        elif name == "raw":
            self.raw_texts, self.raw_Ids, self.mem_mat, self.word_mat, self.word_idx = read_instance(self.raw_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, -1, self.sentence_classification, self.split_token, self.word_idx, self.word_mat, self.mem_mat )
        else:
    ...
    def read_config(self,config_file):
        ...
        the_item = 'eval_script'
        if the_item in config:
            self.eval_script = config[the_item]
        the_item = 'result_dir'
        if the_item in config:
        ...
        the_item = 'label_emb_dir'
        if the_item in config:
            self.label_emb_dir = config[the_item]
        ...
        the_item = 'global_feature_extractor'
        if the_item in config:
            self.global_feature_extractor = config[the_item]
        the_item = 'attention_feature_extractor'
        if the_item in config:
            self.attention_feature_extractor = config[the_item]
        the_item = 'nbest'
        if the_item in config:
        ...
        the_item = 'intNet_layer'
        if the_item in config:
            self.HP_intNet_layer = int(config[the_item])
        the_item = 'intNet_kernel_type'
        if the_item in config:
            self.HP_intNet_kernel_type = int(config[the_item])
        the_item = 'iteration'
        if the_item in config:
        ...
        the_item = 'global_hidden_dim'
        if the_item in config:
            self.global_hidden_dim = int(config[the_item])
        the_item = 'attention_hidden_dim'
        if the_item in config:
            self.attention_hidden_dim = int(config[the_item])
        the_item = 'dropout'
        if the_item in config:
            self.HP_dropout = float(config[the_item])
        the_item = 'rnn_dropout'
        if the_item in config:
            self.rnn_dropout = float(config[the_item])
        the_item = 'lstm_layer'
        if the_item in config:
        ...
        if the_item in config:
            self.global_hidden_dim = int(config[the_item])
        the_item = 'mem_bank_alpha'
        if the_item in config:
            self.mem_bank_alpha = float(config[the_item])
        the_item = 'mem_bank_dropout'
        if the_item in config:
            self.mem_bank_dropout = float(config[the_item])
