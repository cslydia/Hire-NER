    ...
    ### for sequence labeling data format i.e. CoNLL 2003
        for line in in_lines:
            if len(line) > 1:
                ...
                if (len(words) > 0) and ((max_sent_length < 0) or (len(words) < max_sent_length)) :
                    instence_texts.append([words, features, chars, labels, idxs])
                    instence_Ids.append([word_Ids, feature_Ids, char_Ids,label_Ids, idxs])
                words = []
                features = []
                chars = []
                labels = []
        ...
        if (len(words) > 0) and ((max_sent_length < 0) or (len(words) < max_sent_length)) :
            ...
            label_Ids = []
            idxs = []
    return instence_texts, instence_Ids, mem_mat, word_mat, word_idx
