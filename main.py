...
def evaluate(data, model, name, nbest=None, idx=0):
    ...
    for batch_id in range(total_batch):
        ...
        if not instance:
            continue
        batch_word, batch_features, batch_wordlen, batch_wordrecover, batch_char, batch_charlen, batch_charrecover, batch_label, batch_idx, mask  = batchify_with_label(instance, data.HP_gpu, False, data.sentence_classification)
        if nbest and not data.sentence_classification:
            ...
            tag_seq = model(batch_word, batch_wordlen, batch_char, batch_charlen, batch_charrecover, mask, batch_idx)
...
def batchify_sequence_labeling_with_label(input_batch_list, gpu, if_train=True):
    ...
    word_idxs = [sent[4] for sent in input_batch_list]
    word_seq_lengths = torch.LongTensor(list(map(len, words)))
    max_seq_len = word_seq_lengths.max().item()
    word_seq_tensor = torch.zeros((batch_size, max_seq_len), requires_grad =  if_train).long()
    label_seq_tensor = torch.zeros((batch_size, max_seq_len), requires_grad =  if_train).long()
    idx_seq_tensor = torch.zeros((batch_size, max_seq_len), requires_grad =  if_train).long()
    feature_seq_tensors = []
    ...
    for idx, (seq, label, word_idx, seqlen) in enumerate(zip(words, labels, word_idxs, word_seq_lengths)):
        ...
        idx_seq_tensor[idx, :seqlen] = torch.LongTensor(word_idx)
    ...
    idx_seq_tensor = idx_seq_tensor[word_perm_idx]
    ...
    if gpu:
        ...
        idx_seq_tensor = idx_seq_tensor.cuda()
        char_seq_tensor = char_seq_tensor.cuda()
        char_seq_recover = char_seq_recover.cuda()
        mask = mask.cuda()
    return word_seq_tensor,feature_seq_tensors, word_seq_lengths, word_seq_recover, char_seq_tensor, char_seq_lengths, char_seq_recover, label_seq_tensor, idx_seq_tensor, mask
...
def train(data):
    ...
    for idx in range(data.HP_iteration):
        ...
        for batch_id in range(total_batch):
            ...
            if not instance:
                continue
            batch_word, batch_features, batch_wordlen, batch_wordrecover, batch_char, batch_charlen, batch_charrecover, batch_label, batch_idx, mask  = batchify_with_label(instance, data.HP_gpu, True, data.sentence_classification)
            instance_count += 1
            loss, tag_seq = model.calculate_loss(batch_word, batch_wordlen, batch_char, batch_charlen, batch_charrecover, batch_label, mask, batch_idx)
            ...
            mask_memory = (tag_seq == batch_label)
            correct_idx = torch.masked_select(batch_idx, mask_memory * mask)
        ...
        model.word_hidden.mem_bank.make_idx(idx_list)

        # continue
        speed, acc, p, r, f, _,_ = evaluate(data, model, "dev", idx=idx)
        ...
        # ## decode test
        speed, acc, p, r, f, _,_ = evaluate(data, model, "test", idx=idx)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--status', choices=['train', 'decode', 'test'], help='update algorithm', default='train')
